...
\begin{definition}
  An estimator $T(\underline{X})$ is unbiased\index{unbiased} for $\theta$ of $\EE_\theta \left[T(\underline{X})\right] = \theta, \forall \theta \in \Theta$. Otherwise, it is biased\index{biased}. Here $\EE_\theta$ is the expectation if $X_i$ has p.d.f/p.m.f $f(x ; \theta)$. The bias\index{bias} of $T(\underline{X})$ is $\EE_\theta\left[T(\underline{X})\right] - \theta$.
\end{definition}
