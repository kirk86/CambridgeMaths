...
&=& \frac{g(t, \theta)h(\vec{x})}{g(t, \theta)\sum_{\{\vec{x} \mid T\left(\vec{x'}\right)} h(\vec{x'})} \\
&=& \frac{h(\vec{x})}{\sum_{\{\vec{x} \mid T\left(\vec{x'}\right)\}} h(\vec{x'})} \mpunct{.}
\end{IEEEeqnarray*}
...
&=& \underbrace{\PP_\theta \left(\vec{X} = \vec{x}\right) \given T = t)}_{\text{independent of $\theta$ by assumption of sufficiency}}\PP_\theta(T = t)
\end{IEEEeqnarray*}
\end{proof}
